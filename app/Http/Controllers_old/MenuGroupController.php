...
class MenuGroupController extends Controller
{
    ...
    public function index()
    {
        ...
        $records = MenuGroupModels::paginate($this->pagination);
        $param = [
            'page' => $page,
            'tabe_id' => $tabe_id,
            'page_id' => $page_id,
    ...
    public function createData(Request $request)
    {
        try {
            ...
            if (isset($input['type']) && $input['type'] == 'edit') {
                $record = MenuGroupModels::where($this->primary_key, trim(decryptHelper($input['id'])))->first();
    ...
    public function editData(Request $request)
    {
        ...
        try {
            ...
            $record = MenuGroupModels::where($this->primary_key, $data['is_code'])->first();
    ...
    public function deleteData(Request $request)
    {
        ...
        try {
            $data = $request->all();
            $record = MenuGroupModels::where($this->primary_key, $data['code'])->first();
            if (!$record)   return response()->json(['status' => 'warning', 'msg' => 'Record not found in databases ']);
            $record->delete();
            DB::commit();
    ...
    public function ajaxPagination(Request $request)
    {
        try {
            ...
            $records = MenuGroupModels::whereRaw($query)->paginate($this->pagination);
            $page = $this->page;
            $tabe_id = $this->page_id;
            $page_id = $this->page_id;
            $prefix = $this->prefix;
    ...
    public function downLoadExcel(Request $request, $request_table)
    {
        try {
            ...
            $records = MenuGroupModels::whereRaw($query)->get();
            $page = $this->page;
            $page_id = $this->page_id;
            $prefix = $this->prefix;
            $fields = $this->system->getField($this->page_id);
    ...
    public function ImportExcel(Request $request){
        try{
            ...
            foreach($main_data as $sub_collection){
                $insert_record = new MenuGroupModels() ;
                 foreach($sub_collection as $key=>$value){
                     if($this->system->hasColumn($this->prefix,$key)) {
                         $insert_record[$key] = $value;
                     }
                 }
            $record_exist = MenuGroupModels::where($this->primary_key,$insert_record[$this->primary_key ?? ''])->first();
            if($record_exist){
