...
class CourseController extends Controller
{
    ...
    function __construct()
    {
        ...
        $this->modal_path = 'App\Models\App\Models\CourseModels';
        $this->blade_path = 'admin.menu_group.menu_group';
        $this->options = array('cluster' => 'ap1', 'encrypted' => true);
        $this->new_modal = new App\Models\CourseModels();
        $this->blade_path = 'admin.course.course';
    ...
    public function index()
    {
        ...
        $records = App\Models\CourseModels::paginate($this->pagination);
        $param = [
            'page' => $page,
            'tabe_id' => $tabe_id,
            'page_id' => $page_id,
    ...
    public function createData(Request $request)
    {
        try {
            ...
            if (isset($input['type']) && $input['type'] == 'edit') {
                $record = App\Models\CourseModels::where($this->primary_key, trim(decryptHelper($input['id'])))->first();
    ...
    public function submitData(Request $request)
    {
        ...
        try {
            ...
            $primary_key  = $this->primary_key;
            $prefix = $this->prefix;
            $record = new App\Models\CourseModels();
    ...
    public function editData(Request $request)
    {
        ...
        try {
            ...
            $record = App\Models\CourseModels::where($this->primary_key, $data['is_code'])->first();
    ...
    public function deleteData(Request $request)
    {
        ...
        try {
            $data = $request->all();
            $record = App\Models\CourseModels::where($this->primary_key, $data['code'])->first();
            if (!$record)   return response()->json(['status' => 'warning', 'msg' => 'Record not found in databases ']);
            $record->delete();
            DB::commit();
    ...
    public function ajaxPagination(Request $request)
    {
        try {
            ...
            $records = App\Models\CourseModels::whereRaw($query)->paginate($this->pagination);
            $page = $this->page;
            $tabe_id = $this->page_id;
            $page_id = $this->page_id;
            $prefix = $this->prefix;
    ...
    public function downLoadExcel(Request $request, $request_table)
    {
        try {
            ...
            $records = App\Models\CourseModels::whereRaw($query)->get();
            $page = $this->page;
            $page_id = $this->page_id;
            $prefix = $this->prefix;
            $fields = $this->system->getField($this->page_id);
    ...
    public function ImportExcel(Request $request){
        try{
            ...
            foreach($main_data as $sub_collection){
                $insert_record = new App\Models\CourseModels() ;
                 foreach($sub_collection as $key=>$value){
                     if($this->system->hasColumn($this->prefix,$key)) {
                         $insert_record[$key] = $value;
                     }
                 }
            $record_exist = App\Models\CourseModels::where($this->primary_key,$insert_record[$this->primary_key ?? ''])->first();
            if($record_exist){
